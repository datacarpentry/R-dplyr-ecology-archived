...
* download [the data file`"surveys.csv"`](https://github.com/datacarpentry/datacarpentry/raw/master/data/biology/surveys.csv) from Figshare
...
Let's now check the **str**ucture of this `data.frame` in more details with the
function `str()`:
...
As you can see, the columns `species_id` and `sex` are of a special class called
...
In computer's memory, these factors are represented by numbers (1, 2, 3). They 
are better than using simple integer labels because factors are self describing:
